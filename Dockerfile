...
RUN git clone -b REL_17_STABLE --single-branch https://github.com/postgres/postgres &&\
   cd postgres && ./configure --enable-debug  && make && sudo make install 

RUN  git clone -b  citus_health --single-branch https://github.com/akalend/citus 
RUN   cd citus && ./configure  --without-zstd && make && sudo make install 
